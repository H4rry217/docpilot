...
import type { RequestConfirmInput, RequestTextInput } from './useAppDialogs'

type UseWorkspaceShellInput = {
  authStatus: AuthStatus
  requestText: (input: RequestTextInput) => Promise<string | undefined>
  requestConfirm: (input: RequestConfirmInput) => Promise<boolean>
}

export function useWorkspaceShell({ authStatus, requestText, requestConfirm }: UseWorkspaceShellInput) {
  const { t } = useI18n()
